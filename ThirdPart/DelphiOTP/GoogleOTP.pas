...
function CalculateOTP(const Secret: String; const Counter: Integer = -1; const TokenLen : integer = 6): Integer;
function ValidateTOPTTimeBased(const Secret: String; const Token: Integer; const WindowSize: Integer = 4; const TokenLen : integer = 6): Boolean;
function ValidateTOPTCounterBased(const Secret: String; const Token: Integer; const Counter: Integer; const TokenLen : integer = 6): Boolean;
...
begin
  ...
  Result := Key mod Trunc(IntPower(10, TokenLen));
end;
...
begin
  ...
  begin
    if (CalculateOTP(Secret, TestValue, TokenLen) = Token) then
      Result := true;
  end;
end;
...
begin
  TimeStamp := Counter;
  Result := (CalculateOTP(Secret, TimeStamp) = Token);
end;
